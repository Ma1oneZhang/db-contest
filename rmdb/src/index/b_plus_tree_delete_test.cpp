...
public:
	...
	void SetUp() override {
		...
		if (ix_manager_->exists(TEST_FILE_NAME, {meta})) {
			ix_manager_->destroy_index(TEST_FILE_NAME, {meta});
		}
		// 创建测试文件
		ix_manager_->create_index(TEST_FILE_NAME, {meta});
		assert(ix_manager_->exists(TEST_FILE_NAME, {meta}));
		// 打开测试文件
		ih_ = ix_manager_->open_index(TEST_FILE_NAME, {meta});
		assert(ih_ != nullptr);
	}
...
TEST_F(BPlusTreeTests, InsertAndDeleteTest1) {
	...
	assert(buffer_pool_manager_->check_unpin());
	// scan keys by Ixscan
...
TEST_F(BPlusTreeTests, LargeScaleTest) {
	...
	while (add_cnt + del_cnt < scale) {
		...
		if (mock.empty() || dice < insert_prob) {
			...
			del_cnt++;
			// Draw(buffer_pool_manager_.get(),
			// 		 "MixTest2_" + std::to_string(++num) + "_insert" + std::to_string(key) + ".dot");
		}
	}
	Draw(buffer_pool_manager_.get(), "InsertAndDeleteTest3_delete" + std::to_string(123) + ".dot");
