...
public:
     ...
     */
	explicit LRUReplacer(size_t num_frame);
...
private:
	...
	std::vector<std::list<frame_id_t>::iterator> LRU_it_;// frame_id_t -> unpinned pages的frame id
	std::vector<bool> exist_;                            //是否存在于lru表中
	std::vector<bool> is_pined_;                         // is pined
